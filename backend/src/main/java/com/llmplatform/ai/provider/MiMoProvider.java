...
/**
 * MiMo API provider implementation
 * MiMo API is compatible with OpenAI API format
 */
@Slf4j
@Component
public class MiMoProvider implements AIProvider {

    private static final String PROVIDER_NAME = "mimo";

    private final AIProviderConfig config;
    private final RestTemplate restTemplate;
    private final ObjectMapper objectMapper;

    public MiMoProvider(AIProviderConfig config, ObjectMapper objectMapper) {
        this.config = config;
    ...
    public AIResponse generate(AIRequest request) {
        if (!isAvailable()) {
            return AIResponse.failure(PROVIDER_NAME, "MiMo provider is not available or configured");
        }
        ...
        try {
            ...
            log.debug("Sending request to MiMo: {}", url);
            ...
            log.error("MiMo API call failed: {}", e.getMessage(), e);
            return AIResponse.failure(PROVIDER_NAME, "MiMo API call failed: " + e.getMessage());
        }
    }
    ...
    private Map<String, Object> buildRequestBody(AIRequest request, AIProviderConfig.ProviderSettings settings) {
        ...
        // Disable thinking mode for MiMo-V2-Flash
    ...
    private AIResponse parseResponse(String responseBody) {
        try {
            ...
            log.error("Failed to parse MiMo response: {}", e.getMessage(), e);
